...
func AddHouse(request *house.Request) (int, error) {
	...
	var strF string
	for i, s := range request.Facility {
		if i == 0 {
			strF = s
		} else {
			strF += "," + s
...
func GetUserHouse(userName string) ([]*house.Houses, error) {
	...
	//房源信息   一对多查询
	var houses []House
	//GlobalDB.Table("house").Where("userid = (?)", GlobalDB.Table("user").Where("id = ?", user.ID)).Find(&houses)
	GlobalDB.Table("house").Where("user_id = (?)", user.ID).Find(&houses)
	...
	for _, v := range houses {
		...
		houseInfo.ImgUrl = "http://127.0.0.1:9000/image/" + v.Index_image_url
		houseInfo.OrderCount = int32(v.Order_count)
		houseInfo.Price = int32(v.Price)
		houseInfo.RoomCount = int32(v.Room_count)
		houseInfo.UserAvatar = "http://127.0.0.1:9000/image/" + user.Avatar_url
...
func GetHouseDetail(houseId, userName string) (house.DetailData, error) {
	...
	}
	{
		...
		if houseInfo.Index_image_url != "" {
			houseDetail.ImgUrls = append(houseDetail.ImgUrls, "http://127.0.0.1:9000/image/"+houseInfo.Index_image_url)
		}
	}

	//评论在order表
	var orders []OrderHouse
	err := GlobalDB.Table("order_house").Where("house_id = (?)", houseId).Find(&orders).Error
	if err != nil {
	...
	//	return respData, err
	//}
	arr := strings.Split(houseInfo.Facilities, ",")
	for _, s := range arr {
		var faci Facility
		i, _ := strconv.Atoi(s)
		GlobalDB.Table("facility").Where("id = (?)", i).Find(&faci)
		facs = append(facs, faci)
	}
	//fmt.Println(arr)
	for _, v := range facs {
	...
	//}
	err = GlobalDB.Table("house_image").Where("house_id = (?)", houseId).Find(&imgs).Error
	if err != nil {
		fmt.Println("该房屋图片出错", err)
	}
	for _, v := range imgs {
		if len(imgs) != 0 {
			houseDetail.ImgUrls = append(houseDetail.ImgUrls, "http://127.0.0.1:9000/image/"+v.Url)
		}
	}

	//获取房屋所有者信息
	var user User
	err = GlobalDB.Table("user").Where("id = (?)", houseInfo.UserId).Find(&user).Error
	if err != nil {
		fmt.Println("查询房屋所有者信息错误", err)
		return respData, err
	}
	houseDetail.UserName = user.Name
	houseDetail.UserAvatar = "http://127.0.0.1:9000/image/" + user.Avatar_url
...
func GetIndexHouse() ([]*house.Houses, error) {
	...
	var houses []House
	if err := GlobalDB.Limit(5).Where("pass = (?)", 1).Find(&houses).Error; err != nil {
	...
	for _, v := range houses {
		...
		err := GlobalDB.Table("area").Where("id = (?)", v.AreaId).Find(&area).Error
		if err != nil {
			fmt.Println("查询房屋地区信息错误", err)
		}
		err = GlobalDB.Table("user").Where("id = (?)", v.UserId).Find(&user).Error
		if err != nil {
			fmt.Println("查询房屋用户信息错误", err)
		}

		houseTemp.AreaName = area.Name
		houseTemp.Ctime = v.CreatedAt.Format("2006-01-02 15:04:05")
		houseTemp.HouseId = int32(v.ID)
		houseTemp.ImgUrl = "http://127.0.0.1:9000/image/" + v.Index_image_url
		...
		houseTemp.UserAvatar = "http://127.0.0.1:9000/image/" + user.Avatar_url
