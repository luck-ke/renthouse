...
func GetOrderInfo(userName, role string) ([]*userOrder.OrdersData, error) {
	...
	if role == "custom" {
		...
		for _, v := range houses {
			...
			GlobalDB.Table("order_house").Where("house_id", v.ID).Find(&tempOrders)
	...
	for _, v := range orders {
		...
		GlobalDB.Table("house").Where("id", v.HouseId).Find(&house)
		orderTemp.ImgUrl = "http://127.0.0.1:9000/image/" + house.Index_image_url
