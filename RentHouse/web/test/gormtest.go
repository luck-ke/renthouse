...
type House struct {
	...
	//Facilities      []*Facility   ` json:"facilities"`                                //房屋设施   与设施表进行关联
...
func main() {
	//dsn := "root:010927@tcp(127.0.0.1:3306)/test?charset=utf8mb4&parseTime=True&loc=Local"
	...
	db.AutoMigrate(new(House))
	GlobalConn = db

	//////添加字段
	//db.Create(&Student{Name: "小明", Age: 18})
	sqlDB, err := db.DB()
	sqlDB.SetMaxIdleConns(10)
	sqlDB.SetMaxOpenConns(100)
	sqlDB.SetConnMaxLifetime(time.Hour)

	//fmt.Println(GlobalConn.First(&Student{}))
